...
# ==================== 数据加载和预处理 ====================
# [保持原有数据加载代码不变]
df_x_train = pd.read_csv(r"E:\训练集测试集3\arX_train_pubchem.csv", na_values=["?", "NA"])
df_y_train = pd.read_csv(r"E:\训练集测试集3\ary_train_pubchem.csv", na_values=["?", "NA"])
df_x_test = pd.read_csv(r"E:\训练集测试集3\arX_test_pubchem.csv", na_values=["?", "NA"])
df_y_test = pd.read_csv(r"E:\训练集测试集3\ary_test_pubchem.csv", na_values=["?", "NA"])
...
history = final_model.fit(
    ...
    callbacks=[
        ...
        ModelCheckpoint(
            filepath=r"E:\模型8\ar_best_model.h5",
            monitor='val_loss',
            save_best_only=True,
            mode='min',
            verbose=1
...
# 保存最终模型
final_model.save(r"E:\模型8\ar_model_cv.h5")
...
print("\n=== 保存预测结果 ===")
# 保存概率预测
np.save(r"E:\模型8\ar y_pred_proba.npy", y_pred_proba)
# 保存真实标签
np.save(r"E:\模型8\ar y_test.npy", y_test)
# 输出文件信息
print(f"已保存预测概率到: E:/模型8/ar y_pred_proba.npy")
print(f"已保存真实标签到: E:/模型8/ar y_test.npy")
...
plt.tight_layout()
plt.savefig('E:/模型8/ar_roc_curve.pdf', dpi=300, bbox_inches='tight')
plt.close()
...
metrics_df.to_csv('E:/模型8/ar_model_metrics.csv', index=False)
print(f"\n所有指标已保存到: E:/模型8/ar_model_metrics.csv")
