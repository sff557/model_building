...
# 定义保存目录 - 修改为RF专用目录
SAVE_DIR = r"E:\RF_krfp_Results"  # 修改这里为您想要的路径
...
# 针对不平衡数据的参数分布
param_dist = {
    'n_estimators': [200, 300],  # 减少树的数量
    'max_depth': [10, 15, 20],  # 限制深度，避免None
    'min_samples_split': [10, 15, 20],  # 增加
    'min_samples_leaf': [5, 10, 15],  # 增加
    'max_features': ['sqrt', 'log2', 0.5],  # 不使用None
    'bootstrap': [True],
    'criterion': ['gini'],  # 只用gini，简化
    'class_weight': [
        'balanced',
        {0: 0.8, 1: 1.2},  # 温和的权重调整
        {0: 0.7, 1: 1.3},
...
for fold, (train_idx, val_idx) in enumerate(kfold.split(x_train_final, y_train)):
    ...
    # 对训练集使用SMOTE
    smote_fold = SMOTE(random_state=42 + fold)
