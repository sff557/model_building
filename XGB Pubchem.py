...
# 定义保存目录
SAVE_DIR = r"E:\XGBoost_pubchem_Results"
...
model = xgb.XGBClassifier(
    use_label_encoder=False,  # 添加这行
    eval_metric='logloss',  # 同时指定评估指标
...
# 参数搜索
param_dist = {
    'learning_rate': uniform(0.01, 0.3),  # 扩大范围
    'max_depth': [4, 6, 8, 10],  # 增加深度
    'min_child_weight': [1, 2, 3],  # 降低权重
    'subsample': uniform(0.6, 0.4),
    'colsample_bytree': uniform(0.6, 0.4),
    'gamma': uniform(0, 1),  # 降低gamma范围
    'reg_alpha': loguniform(1e-5, 1),  # 降低范围
    'reg_lambda': loguniform(1e-5, 1),  # 降低范围
    'scale_pos_weight': [1, 2, 3, 4]  # 添加正样本权重参数
}
...
# 计算原始阈值(0.5)下的指标
y_pred_original = best_model.predict(x_test_scaled)  # 默认阈值0.5
tn_orig, fp_orig, fn_orig, tp_orig = confusion_matrix(y_test, y_pred_original).ravel()
sp_original = tn_orig / (tn_orig + fp_orig)
se_original = tp_orig / (tp_orig + fn_orig)
...
print(f"\n原始阈值(0.5)性能:")
print(f"AUC: {auc:.4f}")
print(f"ACC: {acc_original:.4f}")
print(f"SE: {se_original:.4f}")
print(f"SP: {sp_original:.4f}")
print(f"F1: {f1_original:.4f}")
print(f"混淆矩阵: TP={tp_orig}, FP={fp_orig}, TN={tn_orig}, FN={fn_orig}")
...
for threshold in thresholds:
    ...
    sp_thresh = tn_thresh / (tn_thresh + fp_thresh) if (tn_thresh + fp_thresh) > 0 else 0
    se_thresh = tp_thresh / (tp_thresh + fn_thresh) if (tp_thresh + fn_thresh) > 0 else 0

    threshold_results.append({
        'threshold': threshold,
        'specificity': sp_thresh,
        'sensitivity': se_thresh,
...
print(f"\n最优阈值: {optimal_threshold['threshold']:.3f}")
print(f"特异度: {optimal_threshold['specificity']:.3f}")
print(f"敏感度: {optimal_threshold['sensitivity']:.3f}")
print(f"F1分数: {optimal_threshold['f1']:.3f}")

# 使用最优阈值进行预测
optimal_threshold_value = optimal_threshold['threshold']
y_pred_optimal = (y_pred_proba >= optimal_threshold_value).astype(int)

# 评估最优阈值下的性能
tn_opt, fp_opt, fn_opt, tp_opt = confusion_matrix(y_test, y_pred_optimal).ravel()
sp_optimal = tn_opt / (tn_opt + fp_opt)
se_optimal = tp_opt / (tp_opt + fn_opt)

print(f"\n优化后性能:")
print(f"特异度: {sp_optimal:.4f} (提高: {sp_optimal - sp_original:.4f})")
print(f"敏感度: {se_optimal:.4f}")
print(f"准确率: {optimal_threshold['accuracy']:.4f}")
print(f"F1分数: {optimal_threshold['f1']:.4f}")
print(f"假阳性(FP): {fp_opt} (原FP: {fp_orig})")
print(f"假阴性(FN): {fn_opt} (原FN: {fn_orig})")

# 使用优化后的阈值作为最终预测
y_pred_final = y_pred_optimal
sp_final = sp_optimal
se_final = se_optimal
acc_final = optimal_threshold['accuracy']
f1_final = optimal_threshold['f1']
tn_final, fp_final, fn_final, tp_final = tn_opt, fp_opt, fn_opt, tp_opt

# 计算MCC
mcc_final = (tp_final * tn_final - fp_final * fn_final) / np.sqrt(
    (tp_final + fp_final) * (tp_final + fn_final) * (tn_final + fp_final) * (tn_final + fn_final)
) if (tp_final + fp_final) * (tp_final + fn_final) * (tn_final + fp_final) * (tn_final + fn_final) != 0 else 0.0

print(f"\n最终模型性能(阈值={optimal_threshold_value:.2f}):")
print(f"AUC: {auc:.4f}")
print(f"ACC: {acc_final:.4f}")
print(f"SE: {se_final:.4f}")
print(f"SP: {sp_final:.4f}")
print(f"F1: {f1_final:.4f}")
print(f"MCC: {mcc_final:.4f}")
print(f"混淆矩阵: TP={tp_final}, FP={fp_final}, TN={tn_final}, FN={fn_final}")
...
plt.title('ROC Curve - XGBoost on chembreta Fingerprints', fontsize=14)
plt.legend(loc="lower right")
plt.grid(True, alpha=0.3)

roc_pdf_path = os.path.join(SAVE_DIR, 'ha xgboost_roc_curve.pdf')
...
plt.title(f'Top {top_n} Feature Importance - XGBoost on chembreta', fontsize=14)
plt.tight_layout()

importance_pdf_path = os.path.join(SAVE_DIR, 'ha xgboost_feature_importance.pdf')
...
# 8. 保存模型和结果
print("\n保存结果...")
joblib.dump(best_model, os.path.join(SAVE_DIR, "ha xgboost_model.pkl"))

# 保存预测结果
np.save(os.path.join(SAVE_DIR, "ha y_test.npy"), y_test)
np.save(os.path.join(SAVE_DIR, "ha y_pred_proba.npy"), y_pred_proba)
...
results.to_csv(os.path.join(SAVE_DIR, 'ha xgboost_results.csv'), index=False)
